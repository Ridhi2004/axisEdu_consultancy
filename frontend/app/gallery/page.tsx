...
//       try {
//         setLoading(true);
//         const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/v1/gallery/`);
...
//   // Process images with fallback categories if needed
//   const processedImages: ProcessedImage[] = useMemo(() => {
//     return images.map((img) => {
//       // Extract filename from URL for alt text
//       const filename = img.images.split('/').pop() || 'gallery-image';
//       const alt = filename.replace(/\.[^/.]+$/, '').replace(/[-_]/g, ' ');
      
//       // Try to determine category from filename or use default
//       let category = "General";
//       const lowerFilename = filename.toLowerCase();
//       if (lowerFilename.includes('campus') || lowerFilename.includes('building')) {
//         category = "Campus";
//       } else if (lowerFilename.includes('class') || lowerFilename.includes('lesson')) {
//         category = "Classes";
//       } else if (lowerFilename.includes('event') || lowerFilename.includes('fair')) {
//         category = "Events";
//       } else if (lowerFilename.includes('counsel') || lowerFilename.includes('advis')) {
//         category = "Counseling";
//       } else if (lowerFilename.includes('student') || lowerFilename.includes('graduate')) {
//         category = "Students Abroad";
//       } else if (lowerFilename.includes('staff') || lowerFilename.includes('team')) {
//         category = "Team";
//       }
...
//     const cats = ["All", ...new Set(processedImages.map((img) => img.category))];
//     return cats;
//   }, [processedImages]);
...
//       activeCategory === "All"
//         ? processedImages
//         : processedImages.filter((img) => img.category === activeCategory),
//     [activeCategory, processedImages]
//   );
...
//             <h3 className="text-red-800 font-semibold text-lg mb-2">Unable to Load Gallery</h3>
...
//                     <span className="text-white/70 text-[10px] mt-1">{image.category}</span>
//                   )}
//                 </div>
...
export default function GalleryPage() {
  ...
  const processedImages: ProcessedImage[] = useMemo(() => {
    return images.map((img) => {
      let imageUrl = img.images;
      
      // Handle different URL formats
      if (imageUrl.startsWith('/media/')) {
        imageUrl = `${process.env.NEXT_PUBLIC_API_URL}${imageUrl}`;
      } else if (!imageUrl.startsWith('http://') && !imageUrl.startsWith('https://')) {
        imageUrl = `${process.env.NEXT_PUBLIC_API_URL}/media/gallery/${imageUrl}`;
      }
      
      const filename = imageUrl.split('/').pop() || 'gallery-image';
      const alt = filename.replace(/\.[^/.]+$/, '').replace(/[-_]/g, ' ');
      ...
      if (lowerFilename.includes('campus') || lowerFilename.includes('building')) {
        category = "Campus";
      } else if (lowerFilename.includes('class') || lowerFilename.includes('lesson') || lowerFilename.includes('ielts')) {
        category = "Classes";
      } else if (lowerFilename.includes('event') || lowerFilename.includes('fair')) {
        category = "Events";
      } else if (lowerFilename.includes('counsel') || lowerFilename.includes('advis') || lowerFilename.includes('visa')) {
        category = "Counseling";
      } else if (lowerFilename.includes('student') || lowerFilename.includes('graduate') || lowerFilename.includes('depart')) {
        category = "Students Abroad";
      } else if (lowerFilename.includes('staff') || lowerFilename.includes('team')) {
        category = "Team";
      }
  ...
  return (
    <div className="bg-slate-50 min-h-screen py-6 lg:py-14">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Gallery Grid - Using img tag instead of Next Image */}
        {filteredImages.length > 0 ? (
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
            {filteredImages.map((image, index) => {
              ...
              return (
                <button
                  key={image.id || index}
                  onClick={() => openLightbox(index)}
                  className="group relative aspect-square rounded-2xl overflow-hidden border border-slate-200 shadow-sm bg-slate-100"
                >
                  {hasError ? (
      ...
      {lightboxIndex !== null && filteredImages.length > 0 && (
        ...
        >
          ...
          <div
            ...
            onClick={(e) => e.stopPropagation()}
          >
            {imageErrors.has(filteredImages[lightboxIndex].src) ? (
              ...
              <img
                ...
                onError={() => handleImageError(filteredImages[lightboxIndex].src)}
              />
            )}
          </div>
