...
// Types — mirror the AbroadDetailSerializer response exactly
type Country = {
  id: number;
  name: string;
  code: string;
...
export default function CountryPage() {
  const params = useParams();
  const slug = params?.country as string; // e.g. "GB" — matches Country.code
  ...
  useEffect(() => {
    ...
    async function fetchCountry() {
      ...
      setError(null);
      try {
        const res = await fetch(`${API_URL}/api/v1/abroad/${slug.toUpperCase()}/`, {
        ...
        const json: CountryDetail = await res.json();
        setData(json);
      } catch (err) {
        if ((err as Error).name !== "AbortError") {
          setError("We couldn't load this page right now. Please try again shortly.");
        }
      } finally {
        setLoading(false);
      }
    ...
    return (
      <div className="min-h-screen bg-white py-16 flex items-center justify-center">
        <div className="flex flex-col items-center gap-3 text-slate-500">
          ...
          <p className="text-sm">Loading country information...</p>
        </div>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="min-h-screen bg-white py-16 flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-[#0B2545] mb-3">Something went wrong</h1>
  ...
  const universities = sortByOrder(data.universities);
  const visaInformation = sortByOrder(data.visa_information);
  const requiredDocuments = sortByOrder(data.required_documents);
    ...
    <div className="bg-slate-50 min-h-screen py-6 lg:py-14">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <nav className="flex items-center gap-2 text-sm text-slate-500 mb-8">
          ...
          <ChevronRight size={14} />
          <Link href="/abroad" className="hover:text-amber-600 transition-colors">Study Abroad</Link>
          <ChevronRight size={14} />
          <span className="text-amber-600 font-medium">{data.country.name}</span>
        </nav>

        <div className="relative rounded-3xl overflow-hidden bg-gradient-to-r from-[#0B2545] via-[#0B2545] to-[#1a3a6b] p-8 lg:p-12 mb-12">
          <div className="relative z-10 max-w-2xl">
            <div className="mb-4">
              <Flag country={data.country.code} size={64} />
            </div>
            <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-white mb-4">{data.hero_title}</h1>
            <p className="text-white/80 text-base sm:text-lg">{data.hero_description}</p>
          </div>
          <div className="pointer-events-none absolute right-0 top-0 w-1/2 h-full opacity-5">
            <GraduationCap size={400} className="absolute -right-20 -top-20 text-white" />
          </div>
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-8">
            <div className="bg-white rounded-3xl p-6 lg:p-8 border border-slate-200 shadow-sm">
              <SectionBadge icon={GraduationCap} label="Overview" />
              <h2 className="mt-3 text-2xl font-bold text-[#0B2545]">{data.overview_title}</h2>
              <div className="mt-5 grid sm:grid-cols-[1fr,220px] gap-6 items-start">
                <p className="text-slate-600 leading-relaxed text-justify">{data.overview_description}</p>
                {data.hero_image && (
                  <div className="relative w-full sm:w-[220px] h-[160px] rounded-2xl overflow-hidden shadow-sm flex-shrink-0">
                    <Image src={data.hero_image} alt={`Study in ${data.country.name}`} fill className="object-cover" />
                  </div>
                )}
              </div>
            </div>
              ...
              <div className="relative bg-[#0B2545] text-white rounded-3xl p-6 lg:p-8 overflow-hidden">
                ...
                <div className="relative mt-6 grid md:grid-cols-2 gap-6">
                  {visaInformation.map((visa) => (
                    <div key={visa.id}>
                      <p className="text-amber-400 text-xs font-semibold uppercase tracking-wider">{visa.title}</p>
                      {visa.visa_type && (
                        <span className="inline-block mt-1.5 bg-white/10 border border-white/10 px-3 py-1 rounded-full text-xs">
                          {visa.visa_type}
                        </span>
                      )}
                      <p className="text-white/90 mt-2 text-sm leading-relaxed">{visa.description}</p>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {universities.length > 0 && (
              <div className="bg-white rounded-3xl p-6 lg:p-8 border border-slate-200 shadow-sm">
                <SectionBadge icon={Building2} label="Institutions" />
                <h2 className="mt-3 text-2xl font-bold text-[#0B2545]">Universities &amp; Colleges We Represent</h2>
                <div className="mt-6 grid md:grid-cols-2 gap-4">
                  {universities.map((uni) => {
                    const CardInner = (
                      <div className="border border-slate-200 rounded-2xl p-4 hover:border-amber-300 hover:shadow-md transition-all duration-300 flex items-start gap-3 h-full">
                        <div className="w-9 h-9 rounded-lg bg-amber-50 flex items-center justify-center flex-shrink-0">
                          <Building2 size={16} className="text-amber-600" />
                        </div>
                        <div>
                          <h3 className="font-semibold text-[#0B2545] text-sm leading-snug">{uni.name}</h3>
                          {uni.location && (
                            <p className="text-xs text-slate-500 flex items-center gap-1 mt-1.5">
                              <MapPin size={12} />
                              {uni.location}
                    ...
                    return uni.website ? (
                      <a key={uni.id} href={uni.website} target="_blank" rel="noopener noreferrer" className="block">
                        {CardInner}
                      </a>
                    ) : (
                      <div key={uni.id}>{CardInner}</div>
                    );
                  })}
                </div>
              </div>
          ...
          <div className="space-y-6">
            {requiredDocuments.length > 0 && (
              <div className="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm">
                <h3 className="font-bold text-[#0B2545] text-lg mb-5">Required Documents</h3>
                <div className="space-y-4">
                  {requiredDocuments.map((doc) => (
                    <div key={doc.id} className="flex items-start gap-3">
                      ...
                      <div>
                        <p className="text-sm font-medium text-[#0B2545]">{doc.title}</p>
            ...
            <div className="relative bg-gradient-to-br from-amber-600 to-amber-700 rounded-3xl p-7 text-center overflow-hidden">
              <div className="pointer-events-none absolute -bottom-10 -left-10 w-40 h-40 rounded-full bg-white/10 blur-2xl" />
              <h4 className="relative text-white font-bold text-lg mb-1.5">Ready to Apply?</h4>
              <p className="relative text-white/90 text-sm mb-5">Get a free consultation with our counselors today.</p>
              <Link
                href="/contact"
                className="relative inline-block bg-white text-amber-600 font-semibold px-6 py-2.5 rounded-full hover:bg-amber-50 transition-colors text-sm shadow-sm"
              >
